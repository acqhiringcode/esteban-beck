<?php $idPage = 'single' ?>
<?php $title = 'Esteban Beck Bolsas' ?>
<?php $description = 'Bolsas de carton, kraft, plastico.' ?>

<?php include './includes/main-nav.php' ?>

<main class="container main-pictures">
    <div class="row">
        <div class="content content-12">
            <h2 class="main-pictures__title">Bolsas de Cartón</h2>
        </div>
    </div>
    <div class="row">
        <div class="content content-3">
            <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-1.jpg" alt="#">
            </figure>
            <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-2.jpg" alt="#">
            </figure>
            <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-12.jpg" alt="#">
            </figure>
        </div>
        <div class="content content-3">
             <figure class="main-pictures__figure">
                 <img class="main-picture__image" src="imgs/carton/image-4.jpg" alt="#">    
            </figure>  
        </div>
        <div class="content content-3">
             <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-7.jpg" alt="#">
            </figure>
            <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-10.jpg" alt="#">
            </figure>
            <figure class="main-pictures__figure">
                <img class="main-picture__image" src="imgs/carton/image-9.jpg" alt="#">
            </figure>
        </div>
    </div>
</main>

<?php include './includes/main-footer.php' ?>